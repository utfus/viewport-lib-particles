...
//! emitter glows additively against the background.
//!
//! The default frame sets no environment map, so no skybox draws. That matters
//! because particle draws happen before the skybox and do not write depth: with
//! a skybox enabled over an empty scene it would paint over them (a transparent-
//! routing concern noted in the plan).
//!
...
#[test]
fn emits_and_draws_a_glow_at_origin() {
    ...
    // Particles spawn at the origin, hold still (no velocity, no forces), and
    // live long enough to accumulate into a bright additive glow.
    let effect = EffectAsset::new("glow")
    ...
    // A few frames so emission accumulates.
    for _ in 0..3 {
    ...
    // The additive glow makes the center much brighter than the background.
    assert!(center > 150, "center too dark; particles did not draw: {center}");
    assert!(
        center > corner + 100,
